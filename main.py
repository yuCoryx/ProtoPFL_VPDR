...
from utils.tools import setup_seed, get_model_info, validate, validate_fedpcl,calibrate_sigma_prototype_rdp
...
def main():
    ...
    args.output_dir = (
        f'logs/{args.exp_name}/{args.dataset}_alpha{args.dirichlet_alpha}/'
        f'N{args.node_num}_T{args.T}_E{args.E}'
    )
    ...
    if args.dataset in ('office_caltech10', 'pacs', 'digits'):
        train_loaders, test_loaders, num_classes = get_domain_fed_loaders(
        ...
        train_loaders, test_loaders, num_classes = get_label_fed_loaders(
            dataset=args.dataset,
            data_root=args.data_root,
    ...
    total_epsilon = float(args.epsilon)
    total_delta = float(args.delta)
    
    if args.noise_add == 'vpp':
        args.vpp_topk_eps = total_epsilon * float(args.vpp_topk_eps_ratio)
        epsilon_for_proto = total_epsilon - args.vpp_topk_eps

        if epsilon_for_proto <= 0:
            raise ValueError(
                f"Invalid privacy split: epsilon={total_epsilon}, "
                f"vpp_topk_eps_ratio={args.vpp_topk_eps_ratio}, "
                f"eps_proto={epsilon_for_proto}"
            )

        logging.info(
            f"[DP] Budget split: "
            f"eps_topk_total={args.vpp_topk_eps:.6f}, "
            f"eps_proto_total={epsilon_for_proto:.6f}, "
            f"eps_total={total_epsilon:.6f}, "
            f"delta={total_delta}"
        )
        
    else:
        args.vpp_topk_eps = 0.0
        epsilon_for_proto = total_epsilon
        logging.info(
            f"[DP] Target prototype-release budget: "
            f"(eps={epsilon_for_proto:.6f}, delta={total_delta})"
        )

    # sigma_ref for prototype release across T rounds
    args.noise_multiplier = calibrate_sigma_prototype_rdp(
        num_releases=args.T,
        target_epsilon=epsilon_for_proto,
        target_delta=total_delta
    )

    logging.info(f"[DP] Calibrated prototype-release noise multiplier: sigma={args.noise_multiplier:.6f}")
    ...
    if args.enable_heterogeneous:
        ...
        server = Node(
            -1,
            None,
            test_loaders.get(-1, next(iter(test_loaders.values()))),
            num_classes,
            server_model,
            server_opt,
            args
        ...
        clients = {}
        for cid in range(args.node_num):
            m = copy.deepcopy(base).to(args.device)
    ...
    for rnd in range(args.T):
        ...
        if args.method in ['fedproto', 'fedpcl', 'fpl', 'fedplvm', 'fedtgp', 'mpft']:
            ...
            if getattr(args, "eval_mia", False):
                try:
                    ...
                    if round_mia_results:
                        all_mia_results.append({
                            'round': rnd + 1,
                            'client_results': round_mia_results
                        })
    ...
    metrics = {
        'args': vars(args),
        'personalization_acc': avg_acc,
        'times': {'total': federated_time}
    }
