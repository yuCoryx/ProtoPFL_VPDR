...
def calibrate_sigma_prototype_rdp(
    num_releases: int,
    ...
    """
    Calibrate Gaussian noise multiplier sigma for repeated prototype releases under RDP.
    """
    if alphas is None:
    ...
    def rdp_to_epsilon(sigma):
        ...
        rdp_epsilons = []
        for alpha in alphas:
            rdp_single = alpha / (2.0 * sigma * sigma)
    ...
    if eps_at_max > target_epsilon:
        raise ValueError(
            f"At sigma={max_sigma}, epsilon={eps_at_max:.6f} is still greater than "
            f"the target epsilon={target_epsilon}. Please use a larger max_sigma, "
            f"relax the privacy budget, or increase the upper bound of alphas."
        )
