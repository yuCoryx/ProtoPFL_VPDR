...
from utils.tools import freeze_layers, calculate_infonce_loss, check_and_fix_nan, move_to_device
from utils.proto import generate_prototypes, add_dp_noise_to_prototypes
...
def compute_dcr_kd(node, args, logits_s, logits_t):
    """
    DCR KD: EMA-teacher logits_t -> student logits_s.
    """
...
def Client_encode(args, client_nodes: dict, select_list: list, flag=None, round_idx=None):
    ...
    if method == "fedpcl" and flag == "validation":
        for idx in select_list:
            ...
            raw_protos, feat_accum, cluster_sizes, sens = generate_prototypes(
                node, args, cluster_method="mean"
            )
    ...
    for idx in select_list:
        ...
        raw_protos, feat_accum, cluster_sizes, sens = generate_prototypes(
            node, args, cluster_method=cluster_method
        )
        logging.info(f"[Local][{method.upper()}][Client {idx}] Prototype generation completed.")

        noised_protos = add_dp_noise_to_prototypes(
            raw_protos,
            sens,
            args,
            node=node,
        ...
        for epoch in range(args.E):
            if args.method == "fedproto":
                ...
                if len(out) == 5:
                    ...
                    logging.info(
                        f"[Local][FEDPROTO] C{idx} E{epoch+1}/{args.E} "
                        f"CE={ce:.4f} MSE={mse:.4f} KD={kd:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
                else:
                    loss, ce, mse, acc = out
                    logging.info(
                        f"[Local][FEDPROTO] C{idx} E{epoch+1}/{args.E} "
                        f"CE={ce:.4f} MSE={mse:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
                ...
                if len(out) == 5:
                    ...
                    logging.info(
                        f"[Local][FEDPCL] C{idx} E{epoch+1}/{args.E} "
                        f"Lg={Lg:.4f} Lp={Lp:.4f} KD={kd:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
                else:
                    loss, Lg, Lp, acc = out
                    logging.info(
                        f"[Local][FEDPCL] C{idx} E{epoch+1}/{args.E} "
                        f"Lg={Lg:.4f} Lp={Lp:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
                ...
                if len(out) == 6:
                    ...
                    logging.info(
                        f"[Local][FEDPLVM] C{idx} E{epoch+1}/{args.E} "
                        f"CE={ce:.4f} Contra={contra:.4f} Corr={corr:.4f} KD={kd:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
                else:
                    loss, ce, contra, corr, acc = out
                    logging.info(
                        f"[Local][FEDPLVM] C{idx} E{epoch+1}/{args.E} "
                        f"CE={ce:.4f} Contra={contra:.4f} Corr={corr:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
                ...
                if kd is not None:
                    logging.info(
                        f"[Local][FPL] C{idx} E{epoch+1}/{args.E} "
                        f"CE={ce:.4f} Contra={contra:.4f} MSE={mse:.4f} KD={kd:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
                else:
                    logging.info(
                        f"[Local][FPL] C{idx} E{epoch+1}/{args.E} "
                        f"CE={ce:.4f} Contra={contra:.4f} MSE={mse:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
                ...
                if len(out) == 5:
                    ...
                    logging.info(
                        f"[Local][FEDTGP] C{idx} E{epoch+1}/{args.E} "
                        f"CE={ce:.4f} ProtoReg={preg:.4f} KD={kd:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
                else:
                    loss, ce, preg, acc = out
                    logging.info(
                        f"[Local][FEDTGP] C{idx} E{epoch+1}/{args.E} "
                        f"CE={ce:.4f} ProtoReg={preg:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
                ...
                if len(out) == 5:
                    ...
                    logging.info(
                        f"[Local][MPFT] C{idx} E{epoch+1}/{args.E} "
                        f"CE={ce:.4f} KD={kd:.4f} DCR_KD={dcr_kd:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
                else:
                    loss, ce, kd, acc = out
                    logging.info(
                        f"[Local][MPFT] C{idx} E{epoch+1}/{args.E} "
                        f"CE={ce:.4f} KD={kd:.4f} Total={loss:.4f} Acc={acc:.2f}%"
                    )
...
def train_fedpcl(node, args):
    ...
    def step(x, y):
        ...
        return loss, sim_g, extras, do_kd
...
def train_fedplvm(node: Node, args):
    ...
    def step(x, y):
        ...
        sim = F.cosine_similarity(
            feat.unsqueeze(1), all_protos.unsqueeze(0), dim=2
        ).clamp(-1 + sim_eps, 1 - sim_eps)
...
def train_fpl(node: Node, args):
    ...
    def step(x, y):
        ...
        contra = (
            calculate_infonce_loss(feat, y, node.glob_cluster_proto, args.contra_T)
            if node.glob_cluster_proto is not None
